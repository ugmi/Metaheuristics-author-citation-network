...
def add_record_additional(mydb, mycursor, all_ids, data, ele, entry, sql):
    ...
    if (int(ele['scopus-id']),) in all_ids['others']:
        ...
        if str(entry[0]) in articles_citing_ele.split(','):
            return all_ids, data
        articles_citing_ele = articles_citing_ele + ',' + str(entry[0])
        mycursor.execute(
        ...
        if (int(ele['scopus-id']),) in all_ids['publications']:
            ...
            if str(ele['scopus-id']) in references.split(','):
                ...
                references = references + ',' + str(ele['scopus-id'])
            else:
        ...
        if 'author' in ele['author-list']:
            for author_info in ele['author-list']['author']:
                ...
                if (int(authid),) not in all_ids['authors']:
                    ...
                    if '@id' in author_info['affiliation']:
                    ...
                    if 'ce:given-name' in author_info:
                        ...
                        first_name = ''
                    values_to_insert.append((authid, author_info['ce:indexed-name'],
                                author_info['ce:surname'], first_name, author_info['ce:initials'],
                                afid, url))
...
sql = {
    'a': ('INSERT INTO authors (id, authname, surname, given_name,'
          ' initials, afids, url) values_to_insertUES (%s, %s, %s, %s, %s, %s, %s)'),
...
mycursor.execute('SELECT eid FROM publications WHERE field LIKE "%PSO"')
